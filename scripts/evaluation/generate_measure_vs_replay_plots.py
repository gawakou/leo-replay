...
def calc_shared_ylim(
    ...
    vals = pd.concat([df1[ycol], df2[ycol]], ignore_index=True).dropna()
    if vals.empty:
        return (0.0, 1.0)
    vmin = float(vals.min())
    vmax = float(vals.max())
    if vmax == vmin:
        margin = max(abs(vmax) * margin_ratio, min_margin)
    else:
        margin = max((vmax - vmin) * margin_ratio, min_margin)
...
def main() -> None:
    ...
    ylim_rtt = calc_shared_ylim(measured_ping, replay_ping, "rtt_ms", margin_ratio=0.05, min_margin=2.0)
    ylim_thr = calc_shared_ylim(measured_iperf, replay_iperf, "throughput_mbps", margin_ratio=0.05, min_margin=0.02)
    ylim_loss = calc_shared_ylim(measured_loss, replay_loss, "loss_pct", margin_ratio=0.10, min_margin=0.1)
    ...
    print(f"RTT ylim       : {ylim_rtt}")
    print(f"Throughput ylim: {ylim_thr}")
    print(f"Loss ylim      : {ylim_loss}")
